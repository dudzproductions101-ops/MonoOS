/*
 * early_init.c – MonoOS PID-1 early initialisation
 *
 ...
 *   2. Reads /proc/cmdline and parses MonoOS-specific boot arguments.
 *   3. Loads the kernel modules listed in /etc/monoos/modules.conf.
 *   4. Sets up the device-mapper nodes for dm-verity.
 *   5. Pivots to the real root filesystem.
 *   6. Execs the main init process (/system/bin/monoos_init).
 *
...
/* ------------------------------------------------------------------ */
#define MONOOS_VERSION          "1.0.0"
#define MODULES_CONF           "/etc/monoos/modules.conf"
...
#define INIT_BINARY            "/system/bin/monoos_init"
...
{
    char buf[256];
    int len = snprintf(buf, sizeof(buf), "<%s>monoos_early_init: %s\n",
                       level, msg);
    if (g_log_fd >= 0 && len > 0)
...
/* ------------------------------------------------------------------ */
/*  Parse /proc/cmdline for monoos.* arguments                         */
/* ------------------------------------------------------------------ */
typedef struct {
    char boot_mode[32];      /* monoos.mode=... */
    int  safe_mode;          /* monoos.safe_mode=1 */
    int  adb_enabled;        /* monoos.adb=1 */
    int  verity_enforcing;   /* monoos.verity=enforcing|permissive */
    char slot_suffix[4];     /* monoos.slot=_a or _b */
} cmdline_opts_t;
...
/* ------------------------------------------------------------------ */
/*  Load kernel modules listed in /etc/monoos/modules.conf             */
/* ------------------------------------------------------------------ */
         ...
         char **argv __attribute__((unused)))
{
    ...
    LOG_INFO("MonoOS " MONOOS_VERSION " early_init starting");
