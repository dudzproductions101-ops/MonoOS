// service_loader.cpp – OneOS service manager / init.rc parser
//
// Reads /etc/oneos/services.conf (an init.rc-compatible subset) and
...
{
    const char *conf = "/etc/oneos/services.conf";
    if (argc > 1) conf = argv[1];

    oneos::ServiceLoader loader(conf);
    if (!loader.load()) {
