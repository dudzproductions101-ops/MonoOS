#!/usr/bin/env bash
# build_bootloader.sh – Compile the MonoOS C bootloader and link it to the
...
(
  cd "$SECURE_BOOT_DIR"
  # --features external_panic_handler: boot_manager already provides the
  # single #[panic_handler] for the combined bootloader.elf link below: see
  # the comment in boot/boot_manager/boot_manager.rs.
  RUSTFLAGS="-C panic=abort -C opt-level=z" \
    "$CARGO" build $CARGO_FLAGS --target "$TARGET_TRIPLE" \
      --features external_panic_handler 2>&1
)
...
$LD \
  ...
  -o "$ELF" \
  "${OBJECTS[@]}" \
  -L"$RUST_LIB_DIR_MGR"   -lmonoos_boot_manager \
  -L"$RUST_LIB_DIR_SB"    -lmonoos_secure_boot \
  --no-undefined
