/*
 * power_manager.c – MonoOS power management kernel module
 *
 ...
 *      UID and exposes it under /proc/monoos/power_stats.
...
/*
 * linux/device.h must precede linux/pm_wakeup.h.  pm_wakeup.h uses
 * `struct device *` in its inline helpers; without the full type in scope
 * the compiler sees an incomplete type and errors out on `dev->power.*`.
 */
...
#include "monoos_power.h" /* via ccflags-y */

MODULE_LICENSE("GPL");
MODULE_AUTHOR("DudasCorp");
MODULE_DESCRIPTION("MonoOS power management extensions");
...
#define MONOOS_SCREEN_ON  1
#define MONOOS_SCREEN_OFF 0
...
int monoos_wakelock_acquire(const char *name, uid_t uid)
{
    ...
    strscpy(wl->name, name, WAKELOCK_NAME_MAX);
    wl->owner_uid   = uid;
...
EXPORT_SYMBOL_GPL(monoos_wakelock_acquire);

/**
 * monoos_wakelock_release – release a named wakelock held by @uid.
 */
...
EXPORT_SYMBOL_GPL(monoos_wakelock_release);
...
EXPORT_SYMBOL_GPL(monoos_power_charge_cpu_ns);
...
EXPORT_SYMBOL_GPL(monoos_power_charge_network);
    ...
    if (action == FB_EVENT_BLANK && event->data) {
        ...
        if (on != g_screen_on) {
            ...
            raw_notifier_call_chain(&g_screen_notifier,
                                    on ? MONOOS_SCREEN_ON : MONOOS_SCREEN_OFF,
                                    NULL);
        }
    }
    return NOTIFY_OK;
...
static int __init monoos_power_init(void)
{
    g_wl_cache = kmem_cache_create("monoos_wl",
                   sizeof(struct monoos_wakelock), 0, SLAB_HWCACHE_ALIGN, NULL);
    if (!g_wl_cache) return -ENOMEM;

    g_drain_cache = kmem_cache_create("monoos_drain",
    ...
    if (g_proc_monoos)
        g_proc_power = proc_create("power_stats", 0444, g_proc_monoos,
                                    &power_stats_fops);

    pr_info("monoos_power: power management extensions loaded\n");
    return 0;
}

static void __exit monoos_power_exit(void)
{
    struct monoos_wakelock *wl;
    struct uid_drain *d;
    ...
    pr_info("monoos_power: unloaded\n");
}

module_init(monoos_power_init);
module_exit(monoos_power_exit);
