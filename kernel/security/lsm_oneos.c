/*
 * lsm_oneos.c – OneOS Linux Security Module
 *
 * Implements the OneOS security policy as a stacked LSM on top of SELinux.
 * Provides:
 *
 *   1. Mandatory permission checks via the OneOS runtime permission system
 ...
 *   4. Audit logging to /proc/oneos/lsm_audit for the security dashboard.
 */
...
MODULE_DESCRIPTION("OneOS Linux Security Module");
...
/* ------------------------------------------------------------------ */
static int oneos_ptrace_access_check(struct task_struct *child,
                                      unsigned int mode)
{
...
/* ------------------------------------------------------------------ */
static int oneos_task_kill(struct task_struct *p, struct kernel_siginfo *info,
...
/*  caller's process doesn't hold the matching OneOS permission.      */
/* ------------------------------------------------------------------ */
static int oneos_file_open(struct file *file)
{
    const char *name;
    u32 required_perm = 0;
    ...
    if (required_perm) {
        bool ok = oneos_proc_has_perm(current->pid, required_perm);
...
/*  LSM registration                                                   */
/* ------------------------------------------------------------------ */
static struct security_hook_list oneos_lsm_hooks[] __lsm_ro_after_init = {
    LSM_HOOK_INIT(ptrace_access_check, oneos_ptrace_access_check),
    LSM_HOOK_INIT(task_kill,           oneos_task_kill),
    LSM_HOOK_INIT(file_open,           oneos_file_open),
};
...
static int __init oneos_lsm_init(void)
{
    security_add_hooks(oneos_lsm_hooks, ARRAY_SIZE(oneos_lsm_hooks),
                       "oneos");

    g_proc_oneos = proc_mkdir("oneos", NULL);
    if (g_proc_oneos) {
        g_proc_audit = proc_create("lsm_audit", 0400, g_proc_oneos,
                                    &lsm_audit_fops);
        g_proc_stats = proc_create("lsm_stats", 0444, g_proc_oneos,
                                    &lsm_stats_fops);
    }

    pr_info("oneos_lsm: security module loaded (%zu hooks)\n",
            ARRAY_SIZE(oneos_lsm_hooks));
    return 0;
}

static void __exit oneos_lsm_exit(void)
{
    if (g_proc_audit) proc_remove(g_proc_audit);
    if (g_proc_stats) proc_remove(g_proc_stats);
    if (g_proc_oneos) proc_remove(g_proc_oneos);
    pr_info("oneos_lsm: unloaded\n");
}

module_init(oneos_lsm_init);
module_exit(oneos_lsm_exit);
