/*
 * vfs_overlay.c – OneOS VFS privacy overlay
 *
 ...
 *      enforces OneOS runtime permissions before allowing access.
 *
 *   2. Provides a synthetic /proc/oneos/fs_events ring buffer that
...
MODULE_DESCRIPTION("OneOS VFS privacy overlay");
...
struct sensitive_path {
    const char *prefix;
    u32         required_perm;   /* ONEOS_PERM_* from process.c      */
...
static const struct sensitive_path sensitive_paths[] = {
    { "/dev/video",        ONEOS_PERM_CAMERA,   true  },
    { "/dev/camera",       ONEOS_PERM_CAMERA,   true  },
    { "/dev/snd/",         ONEOS_PERM_MIC,      true  },
    { "/dev/dsp",          ONEOS_PERM_MIC,      true  },
    { "/sys/bus/iio/",     ONEOS_PERM_LOCATION, false },
    { "/proc/net/",        ONEOS_PERM_STORAGE,  false },
};
...
/*  against sensitive_paths and check the caller's OneOS permission.  */
/* ------------------------------------------------------------------ */
extern bool oneos_proc_has_perm(pid_t pid, u32 perm_bit);

static int oneos_inode_permission(struct inode *inode, int mask)
{
    ...
    for (i = 0; i < N_SENSITIVE_PATHS; i++) {
        if (!strncmp(path_str, sensitive_paths[i].prefix,
            ...
            bool ok = oneos_proc_has_perm(current->pid,
...
/* ------------------------------------------------------------------ */
/*  /proc/oneos/fs_events  – readable ring buffer for privacy daemon  */
/* ------------------------------------------------------------------ */
...
/*  LSM registration                                                   */
/* ------------------------------------------------------------------ */
static struct security_hook_list oneos_hooks[] __lsm_ro_after_init = {
    LSM_HOOK_INIT(inode_permission, oneos_inode_permission),
};

static struct lsm_blob_sizes oneos_blob_sizes __initdata = {};

static int __init oneos_vfs_overlay_init_lsm(void)
{
    security_add_hooks(oneos_hooks, ARRAY_SIZE(oneos_hooks), "oneos_vfs");
    return 0;
}
...
static int __init oneos_vfs_init(void)
{
    g_proc_oneos  = proc_mkdir("oneos", NULL);
    if (g_proc_oneos) {
        g_proc_events = proc_create("fs_events", 0400, g_proc_oneos,
                                     &fs_events_fops);
        g_proc_stats  = proc_create("vfs_stats", 0444, g_proc_oneos,
                                     &vfs_stats_fops);
    }

    oneos_vfs_overlay_init_lsm();
    pr_info("oneos_vfs: VFS privacy overlay loaded (%zu sensitive paths)\n",
            N_SENSITIVE_PATHS);
    return 0;
}

static void __exit oneos_vfs_exit(void)
{
    ...
    pr_info("oneos_vfs: unloaded\n");
}

module_init(oneos_vfs_init);
module_exit(oneos_vfs_exit);
