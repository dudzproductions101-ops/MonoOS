/*
 * process.c – OneOS process lifecycle extensions
 *
 ...
 *   4. Death notification:  notify the OneOS permission service when a
...
MODULE_DESCRIPTION("OneOS process lifecycle extensions");
MODULE_VERSION("1.0.0");

/* ------------------------------------------------------------------ */
/*  App privacy flags (bitmask per process)                            */
/* ------------------------------------------------------------------ */
#define ONEOS_PERM_CAMERA      BIT(0)
#define ONEOS_PERM_MIC         BIT(1)
#define ONEOS_PERM_LOCATION    BIT(2)
#define ONEOS_PERM_CONTACTS    BIT(3)
#define ONEOS_PERM_STORAGE     BIT(4)
#define ONEOS_PERM_PHONE       BIT(5)
#define ONEOS_PERM_BLUETOOTH   BIT(6)
#define ONEOS_PERM_NFC         BIT(7)

/* ------------------------------------------------------------------ */
/*  Per-process record                                                 */
/* ------------------------------------------------------------------ */
#define ONEOS_PROC_HASH_BITS  8   /* 256 buckets */

struct oneos_proc_record {
    pid_t             pid;
    uid_t             uid;
...
static DEFINE_HASHTABLE(g_proc_table, ONEOS_PROC_HASH_BITS);
...
/*  Record management                                                  */
/* ------------------------------------------------------------------ */
static struct oneos_proc_record *proc_record_alloc(pid_t pid, uid_t uid,
                                                    const char *comm)
{
    struct oneos_proc_record *rec;
    ...
    strlcpy(rec->comm, comm, sizeof(rec->comm));
    return rec;
}
...
{
    struct oneos_proc_record *rec =
        container_of(head, struct oneos_proc_record, rcu);
...
/**
 * oneos_proc_register – register a new process in the OneOS table.
 *
 * Called from a task_newtask tracepoint handler immediately after fork.
 */
int oneos_proc_register(pid_t pid, uid_t uid, const char *comm)
{
    struct oneos_proc_record *rec;
    unsigned long flags;
...
EXPORT_SYMBOL_GPL(oneos_proc_register);

/**
 * oneos_proc_unregister – remove a process record on exit.
 *
 * Revokes all active permissions and notifies the permission service.
 */
void oneos_proc_unregister(pid_t pid)
{
    struct oneos_proc_record *rec;
    unsigned long flags;
...
EXPORT_SYMBOL_GPL(oneos_proc_unregister);

/**
 * oneos_proc_grant_perm – grant a hardware permission to a process.
 * Returns 0 on success, -ESRCH if pid not found.
 */
int oneos_proc_grant_perm(pid_t pid, u32 perm_bit)
{
    struct oneos_proc_record *rec;
    int ret = -ESRCH;
...
EXPORT_SYMBOL_GPL(oneos_proc_grant_perm);

/**
 * oneos_proc_revoke_perm – revoke a hardware permission from a process.
 */
int oneos_proc_revoke_perm(pid_t pid, u32 perm_bit)
{
    struct oneos_proc_record *rec;
    int ret = -ESRCH;
...
EXPORT_SYMBOL_GPL(oneos_proc_revoke_perm);

/**
 * oneos_proc_has_perm – query whether a process holds a given permission.
 */
bool oneos_proc_has_perm(pid_t pid, u32 perm_bit)
{
    struct oneos_proc_record *rec;
    bool has = false;
...
EXPORT_SYMBOL_GPL(oneos_proc_has_perm);

/* ------------------------------------------------------------------ */
/*  /proc/oneos/processes                                              */
/* ------------------------------------------------------------------ */
...
{
    oneos_proc_unregister(current->pid);
    return 0;
}
...
/*  Module init / exit                                                 */
/* ------------------------------------------------------------------ */
static struct proc_dir_entry *g_proc_oneos_dir;
static struct proc_dir_entry *g_proc_processes;

static int __init oneos_process_init(void)
{
    int ret;

    g_proc_cache = kmem_cache_create("oneos_proc",
                                      sizeof(struct oneos_proc_record),
    ...
    if (ret) {
        pr_warn("oneos_process: kprobe on do_exit failed (%d), "
                "unregister on exit disabled\n", ret);
    }

    g_proc_oneos_dir = proc_mkdir("oneos", NULL);
    if (g_proc_oneos_dir) {
        g_proc_processes = proc_create("processes", 0444,
                                        g_proc_oneos_dir, &proc_list_fops);
    }

    pr_info("oneos_process: process lifecycle extensions loaded\n");
    return 0;
}

static void __exit oneos_process_exit(void)
{
    if (g_proc_processes) proc_remove(g_proc_processes);
    if (g_proc_oneos_dir) proc_remove(g_proc_oneos_dir);
    ...
    pr_info("oneos_process: unloaded\n");
}

module_init(oneos_process_init);
module_exit(oneos_process_exit);
