...
/*
 * Include uapi/linux/sched/types.h for struct sched_attr (needed by
 * sched_setattr_nocheck for SCHED_BATCH / SCHED_IDLE which have no
 * dedicated exported helper in this kernel).  Note: linux/sched/types.h
 * only defines struct task_cputime; the *uapi* header defines sched_attr.
 */
#include <uapi/linux/sched/types.h>
#include <linux/pid.h>          /* find_get_pid, get_pid_task, put_pid */
...
#include "monoos_sched.h" /* via ccflags-y */
...
/* Thread classification (typedef lives in monoos_sched.h) */
 ...
 *
 * Uses only EXPORT_SYMBOL_GPL helpers to remain loadable as an out-of-tree
 * module:
 *   - find_get_pid() + get_pid_task()  instead of find_task_by_vpid()
 *   - sched_set_fifo() / sched_set_normal() / sched_setattr_nocheck()
 *     instead of sched_setscheduler_nocheck()
 */
int monoos_sched_classify(pid_t tid, thread_class_t cls)
{
    ...
    /* Look up task using exported find_get_pid + get_pid_task. */
    ...
    switch (class_params[cls].policy) {
    case SCHED_FIFO:
        if (class_params[cls].rt_prio >= 50)
            sched_set_fifo(task);
        else
            sched_set_fifo_low(task);
        break;
    case SCHED_NORMAL:
        sched_set_normal(task, class_params[cls].nice);
        break;
    default:
        /* SCHED_BATCH and SCHED_IDLE: use the generic sched_setattr_nocheck */
        attr.sched_policy   = class_params[cls].policy;
        attr.sched_nice     = class_params[cls].nice;
        attr.sched_flags    = 0;
