...
MODULE_DESCRIPTION("OneOS scheduler extensions");
...
/* ------------------------------------------------------------------ */
static struct oneos_sched_thread *sched_record_find(pid_t tid)
{
    struct oneos_sched_thread *rec;
...
{
    struct oneos_sched_thread *rec =
        container_of(head, struct oneos_sched_thread, rcu);
...
/**
 * oneos_sched_classify – classify thread @tid into @cls and apply the
 * corresponding scheduler policy / nice value.
 */
int oneos_sched_classify(pid_t tid, thread_class_t cls)
{
    struct oneos_sched_thread *rec;
...
EXPORT_SYMBOL_GPL(oneos_sched_classify);

/**
 * oneos_sched_set_frame_target – register a frame period for @tid so the
 ...
 */
int oneos_sched_set_frame_target(pid_t tid, u32 fps)
{
    struct oneos_sched_thread *rec;
    unsigned long flags;
...
EXPORT_SYMBOL_GPL(oneos_sched_set_frame_target);

/**
 * oneos_sched_frame_begin – called by the Wayland compositor at the start
 * of each frame to boost the render thread for the frame window.
 */
void oneos_sched_frame_begin(pid_t tid)
{
    struct oneos_sched_thread *rec;
...
EXPORT_SYMBOL_GPL(oneos_sched_frame_begin);

/**
 * oneos_sched_unregister – remove a thread from the scheduler extension table.
 */
void oneos_sched_unregister(pid_t tid)
{
    struct oneos_sched_thread *rec;
    unsigned long flags;
...
EXPORT_SYMBOL_GPL(oneos_sched_unregister);
...
static int __init oneos_sched_init(void)
{
    g_sched_cache = kmem_cache_create("oneos_sched",
                                       sizeof(struct oneos_sched_thread),
    ...
    if (g_proc_oneos)
        g_proc_sched = proc_create("sched", 0644, g_proc_oneos,
                                    &sched_info_fops);

    pr_info("oneos_sched: scheduler extensions loaded (target %u fps)\n",
            g_target_fps);
    return 0;
}

static void __exit oneos_sched_exit(void)
{
    struct oneos_sched_thread *rec;
    ...
    pr_info("oneos_sched: unloaded\n");
}

module_init(oneos_sched_init);
module_exit(oneos_sched_exit);
