/*
 * mm.c – OneOS memory management extensions
 *
 * Sits above the Linux kernel's page allocator.  Provides:
 *   1. oneos_alloc_pages()  – policy-aware page allocation with NUMA and
 *      CMA hints for mobile workloads.
 *   2. oneos_free_pages()   – complementary release path.
 *   3. oneos_mmap_anon()    – anonymous mapping with memory-pressure hints.
 *   4. Memory pressure notification hooks consumed by the OOM daemon.
 *
 * Built as an out-of-tree kernel module (oneos_mm.ko).
 */
...
MODULE_DESCRIPTION("OneOS memory management extensions");
...
/* ------------------------------------------------------------------ */
#define ONEOS_CMA_THRESHOLD_PAGES  1024   /* 4 MiB */
#define ONEOS_GFP_DEFAULT          (GFP_KERNEL | __GFP_HIGHMEM | __GFP_NOWARN)
#define ONEOS_GFP_RECLAIM          (GFP_KERNEL | __GFP_RECLAIM | __GFP_NOWARN)
...
/* ------------------------------------------------------------------ */
struct page *oneos_alloc_pages(unsigned int order, gfp_t extra_flags)
{
    gfp_t flags = ONEOS_GFP_DEFAULT | extra_flags;
    struct page *page;
    ...
    pr_warn("oneos_mm: alloc_pages order=%u failed\n", order);
    return NULL;
}
EXPORT_SYMBOL_GPL(oneos_alloc_pages);

/* ------------------------------------------------------------------ */
/*  oneos_free_pages                                                   */
/* ------------------------------------------------------------------ */
void oneos_free_pages(struct page *page, unsigned int order)
{
    if (unlikely(!page))
        return;
...
EXPORT_SYMBOL_GPL(oneos_free_pages);

/* ------------------------------------------------------------------ */
/*  oneos_vmalloc – virtually-contiguous allocation with custom flags  */
/* ------------------------------------------------------------------ */
void *oneos_vmalloc(size_t size, gfp_t gfp)
{
...
EXPORT_SYMBOL_GPL(oneos_vmalloc);
...
static unsigned long oneos_shrink_count(struct shrinker *shrink,
                                         struct shrink_control *sc)
{
    return (unsigned long)atomic_long_read(&g_cache_objects);
}

static unsigned long oneos_shrink_scan(struct shrinker *shrink,
                                        struct shrink_control *sc)
{
    long freed = 0;
...
static struct shrinker oneos_shrinker = {
    .count_objects = oneos_shrink_count,
    .scan_objects  = oneos_shrink_scan,
    .seeks         = DEFAULT_SEEKS,
};

/* ------------------------------------------------------------------ */
/*  /proc/oneos/mm  – debug statistics                                 */
/* ------------------------------------------------------------------ */
...
/*  Module init / exit                                                 */
/* ------------------------------------------------------------------ */
static int __init oneos_mm_init(void)
{
    int ret;

    ret = register_shrinker(&oneos_shrinker, "oneos-mm");
    if (ret) {
        pr_err("oneos_mm: failed to register shrinker: %d\n", ret);
        return ret;
    }

    g_proc_dir = proc_mkdir("oneos", NULL);
    if (g_proc_dir)
        g_proc_mm = proc_create("mm", 0444, g_proc_dir, &mm_stats_fops);

    pr_info("oneos_mm: memory management extensions loaded\n");
    return 0;
}

static void __exit oneos_mm_exit(void)
{
    if (g_proc_mm)
        proc_remove(g_proc_mm);
    if (g_proc_dir)
        proc_remove(g_proc_dir);
    unregister_shrinker(&oneos_shrinker);
    pr_info("oneos_mm: unloaded\n");
}

module_init(oneos_mm_init);
module_exit(oneos_mm_exit);
