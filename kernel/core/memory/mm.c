...
#include "monoos_mm.h" /* via ccflags-y */
...
/*                                                                     */
/*  Linux 6.x changed the shrinker API:                                */
/*    Old (≤5.x): static struct shrinker + register_shrinker(s, name) */
/*    New (6.x+): shrinker_alloc(flags, fmt) / shrinker_register(s) / */
/*                shrinker_free(s)                                     */
/*  SHRINK_DONE was never a public symbol; SHRINK_STOP is the correct  */
/*  constant to return when no further progress is possible.           */
/* ------------------------------------------------------------------ */
...
/* Pointer-based shrinker required by the Linux 6.x API. */
                                        ...
                                        struct shrink_control *sc)
{
    ...
    if (to_free <= 0)
        return SHRINK_STOP;   /* was SHRINK_DONE (never exported); correct constant is SHRINK_STOP */
...
{
    /* Linux 6.x shrinker API: allocate a dynamic shrinker, fill it, then
     * register it.  The old register_shrinker(&static_struct, name) was
     * removed; shrinker_alloc/shrinker_register/shrinker_free are the
     * replacements.  SHRINKER_NUMA_AWARE is optional; omitting it keeps
     * us compatible with kernels that don't define it. */
    g_shrinker = shrinker_alloc(0, "monoos-mm");
    if (!g_shrinker) {
        pr_err("monoos_mm: failed to allocate shrinker\n");
        return -ENOMEM;
    }
    g_shrinker->count_objects = monoos_shrink_count;
    g_shrinker->scan_objects  = monoos_shrink_scan;
