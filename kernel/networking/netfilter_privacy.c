/*
 * netfilter_privacy.c – OneOS network privacy filter (kernel module)
 *
 * Registers Netfilter hooks to:
 *   1. Block outbound traffic from apps that do not hold ONEOS_PERM_NETWORK.
 ...
 *   4. Emit connection events to /proc/oneos/net_events for the
 *      privacy dashboard.
 */
...
MODULE_DESCRIPTION("OneOS network privacy filter");
MODULE_VERSION("1.0.0");

/* ------------------------------------------------------------------ */
/*  Compile-time tracker IP blocklist (illustrative; a tiny subset)   */
/*  Runtime list is loaded from /etc/oneos/tracker_ips via Netlink.   */
/* ------------------------------------------------------------------ */
...
#define ONEOS_PERM_NETWORK  0x0200U
...
/* ------------------------------------------------------------------ */
static unsigned int oneos_nf_ipv4_out(void *priv,
                                       struct sk_buff *skb,
                                       const struct nf_hook_state *state)
{
    ...
    /* Tracker check even without a rule (global setting). */
    if (!block && ip_is_blocked_tracker(iph->daddr)) {
        atomic64_inc(&g_tracker_hits);
        /* Log but do not block unless per-app rule says so. */
...
static const struct nf_hook_ops oneos_nf_hooks[] = {
    {
        .hook      = oneos_nf_ipv4_out,
        .pf        = NFPROTO_IPV4,
...
/* ------------------------------------------------------------------ */
int oneos_net_set_rule(uid_t uid, bool allow, bool block_trackers,
                        bool vpn_only)
{
    struct fw_rule *rule;
    unsigned long flags;
...
EXPORT_SYMBOL_GPL(oneos_net_set_rule);
...
static int __init oneos_netfilter_init(void)
{
    int ret = nf_register_net_hooks(&init_net, oneos_nf_hooks,
                                     ARRAY_SIZE(oneos_nf_hooks));
    if (ret) {
        pr_err("oneos_net: nf_register failed: %d\n", ret);
        return ret;
    }

    g_proc_oneos  = proc_mkdir("oneos", NULL);
    if (g_proc_oneos) {
        g_proc_events = proc_create("net_events", 0400, g_proc_oneos,
                                     &net_events_fops);
        g_proc_stats  = proc_create("net_stats",  0444, g_proc_oneos,
                                     &net_stats_fops);
    }

    pr_info("oneos_net: network privacy filter loaded (%zu tracker IPs)\n",
            N_BLOCKED_IPS);
    return 0;
}

static void __exit oneos_netfilter_exit(void)
{
    nf_unregister_net_hooks(&init_net, oneos_nf_hooks,
                             ARRAY_SIZE(oneos_nf_hooks));
    ...
    pr_info("oneos_net: unloaded\n");
}

module_init(oneos_netfilter_init);
module_exit(oneos_netfilter_exit);
