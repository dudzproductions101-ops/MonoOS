// render_engine.cpp – MonoOS Wayland/Vulkan Render Engine Framework Layer
//
...
extern "C" {
    int monoos_render_init(uint32_t w, uint32_t h, uint32_t hz) {
        return monoos::gfx::RenderEngine::instance().init(nullptr, nullptr, w, h, hz) ? 0 : -1;
    }
    void monoos_render_shutdown() {
        monoos::gfx::RenderEngine::instance().shutdown();
    }
}
