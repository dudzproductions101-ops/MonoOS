//! package_installer.rs – MonoOS Framework Package Installer
//!
//! Validates and installs OPK (MonoOS Package) files.  An OPK is a
...
mod tests {
    ...
    fn install_and_uninstall() {
        let tmp = std::env::temp_dir().join("monoos_pkg_test");
        ...
        assert!(inst.is_installed("com.monoos.myapp"));
        assert_eq!(inst.uninstall("com.monoos.myapp"), UninstallResult::Success);
        assert!(!inst.is_installed("com.monoos.myapp"));
    }
}
