// wifi_hal.cpp – OneOS Hardware Abstraction Layer: WIFI
//
...
int oneos_wifi_hal_init() {
    return oneos::hal::WifiHalManager::instance().init() ? 0 : -1;
}

int oneos_wifi_hal_self_test() {
    auto *h = oneos::hal::WifiHalManager::instance().get();
    return (h && h->self_test()) ? 0 : -1;
}

int oneos_wifi_hal_version() {
    auto *h = oneos::hal::WifiHalManager::instance().get();
