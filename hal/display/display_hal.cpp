// display_hal.cpp – OneOS Hardware Abstraction Layer: DISPLAY
//
...
int oneos_display_hal_init() {
    return oneos::hal::DisplayHalManager::instance().init() ? 0 : -1;
}

int oneos_display_hal_self_test() {
    auto *h = oneos::hal::DisplayHalManager::instance().get();
    return (h && h->self_test()) ? 0 : -1;
}

int oneos_display_hal_version() {
    auto *h = oneos::hal::DisplayHalManager::instance().get();
