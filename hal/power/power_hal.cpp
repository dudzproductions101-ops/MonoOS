// power_hal.cpp – MonoOS Hardware Abstraction Layer: POWER
//
...
int monoos_power_hal_init() {
    return monoos::hal::PowerHalManager::instance().init() ? 0 : -1;
}

int monoos_power_hal_self_test() {
    auto *h = monoos::hal::PowerHalManager::instance().get();
    return (h && h->self_test()) ? 0 : -1;
}

int monoos_power_hal_version() {
    auto *h = monoos::hal::PowerHalManager::instance().get();
