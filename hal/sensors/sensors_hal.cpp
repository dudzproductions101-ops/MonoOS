// sensors_hal.cpp – OneOS Hardware Abstraction Layer: SENSORS
//
...
int oneos_sensors_hal_init() {
    return oneos::hal::SensorsHalManager::instance().init() ? 0 : -1;
}

int oneos_sensors_hal_self_test() {
    auto *h = oneos::hal::SensorsHalManager::instance().get();
    return (h && h->self_test()) ? 0 : -1;
}

int oneos_sensors_hal_version() {
    auto *h = oneos::hal::SensorsHalManager::instance().get();
