// bluetooth_hal.cpp – OneOS Hardware Abstraction Layer: BLUETOOTH
//
...
int oneos_bluetooth_hal_init() {
    return oneos::hal::BluetoothHalManager::instance().init() ? 0 : -1;
}

int oneos_bluetooth_hal_self_test() {
    auto *h = oneos::hal::BluetoothHalManager::instance().get();
    return (h && h->self_test()) ? 0 : -1;
}

int oneos_bluetooth_hal_version() {
    auto *h = oneos::hal::BluetoothHalManager::instance().get();
