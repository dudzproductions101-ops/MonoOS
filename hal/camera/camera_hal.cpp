// camera_hal.cpp – OneOS Hardware Abstraction Layer: CAMERA
//
...
int oneos_camera_hal_init() {
    return oneos::hal::CameraHalManager::instance().init() ? 0 : -1;
}

int oneos_camera_hal_self_test() {
    auto *h = oneos::hal::CameraHalManager::instance().get();
    return (h && h->self_test()) ? 0 : -1;
}

int oneos_camera_hal_version() {
    auto *h = oneos::hal::CameraHalManager::instance().get();
