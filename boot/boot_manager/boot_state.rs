//! boot_state.rs – Persistent and transient boot state for MonoOS
//!
...
/// Length of `_reserved`, derived so the struct totals exactly
/// `BOOT_STATE_BLOCK_SIZE` bytes. Keeping this computed (rather than a
/// hand-picked literal) prevents the on-disk layout from silently drifting
/// out of sync if the string fields above are ever resized.
///
/// The trailing `- 4` accounts for compiler-inserted alignment padding at
/// the end of the struct (it contains a `u32` field, so the whole
/// `#[repr(C)]` struct is 4-byte aligned and its size is rounded up to a
/// multiple of 4); verified against the real layout via static_assert_size.
pub const BOOT_STATE_RESERVED_LEN: usize = BOOT_STATE_BLOCK_SIZE
    - (4 + 2 + 4 + 2 + BOOT_COMMAND_LEN + BOOT_STATUS_LEN + BOOT_RECOVERY_ARG_LEN + 1 + 1)
    - 4;
...
pub struct BootStateBlock {
    ...
    pub _reserved:    [u8; BOOT_STATE_RESERVED_LEN],
}
...
impl BootStateBlock {
    ...
    pub fn new_blank() -> Self {
        let mut b = BootStateBlock {
            ...
            fail_count:   0,
            fail_limit:   3,
            _reserved:    [0u8; BOOT_STATE_RESERVED_LEN],
        };
        b.update_crc();
        b
    }
...
impl BootState {
    ...
    pub fn from_block(block: &BootStateBlock) -> Self {
        BootState {
            ...
            status_msg:   heapless::String::try_from(block.status_str())
                              .unwrap_or_else(|_| heapless::String::new()),
            recovery_arg: heapless::String::try_from(block.recovery_arg_str())
                              .unwrap_or_else(|_| heapless::String::new()),
