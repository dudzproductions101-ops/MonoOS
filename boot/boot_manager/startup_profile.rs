...
pub const PROFILE_NORMAL: StartupProfile = StartupProfile {
    ...
    kernel_args: KernelArgs {
        args: &["monoos.mode=normal", "quiet", "loglevel=3"],
    },
...
pub const PROFILE_FASTBOOT: StartupProfile = StartupProfile {
    ...
    kernel_args: KernelArgs {
        args: &["monoos.mode=fastboot", "loglevel=7"],
    },
...
pub const PROFILE_SAFE: StartupProfile = StartupProfile {
    ...
    kernel_args: KernelArgs {
        args: &["monoos.mode=safe", "monoos.safe_mode=1", "loglevel=4"],
    },
    require_display: true,
    early_network:   false,
    adb_allowed:     false,
...
///
/// `adb_enabled` reflects the user's persistent "Enable ADB" developer
/// setting. Recovery/Fastboot/Diagnostics always allow ADB regardless of
/// this flag (it's required for their purpose); Normal and Safe mode only
/// allow it when the user has explicitly opted in, in keeping with MonoOS's
/// privacy-by-default posture.
pub fn select_profile(cmd: BootCommand, adb_enabled: bool) -> StartupProfile {
    let mut profile = match cmd {
    ...
    if matches!(cmd, BootCommand::Normal | BootCommand::SafeMode) {
        profile.adb_allowed = adb_enabled;
    }
    profile
}
