...
//!
//! MonoOS uses an A/B (seamless) partition scheme similar to Android A/B OTA.
...
use crate::partition_manager::SlotSuffix;
...
impl AbControl {
    ...
    pub fn is_valid(&self) -> bool {
        if self.magic != AB_CONTROL_MAGIC {
            return false;
        }
        let saved = self.crc32;
        saved == self.compute_crc()
    }

    /// Compute CRC32 over every field except the trailing `crc32` field
    /// itself, i.e. bytes `[0..offset_of(crc32)]`.
    pub fn compute_crc(&self) -> u32 {
        ...
        let crc_offset = core::mem::size_of::<Self>() - core::mem::size_of::<u32>();
        crate::boot_state::crc32_ieee(&raw[..crc_offset])
    }

    /// Recompute and store the CRC32 in `self.crc32`. Must be called before
    /// persisting this struct back to the MISC partition.
...
impl KernelSelector {
    ...
    /// Check whether a proposed kernel version satisfies rollback
    /// protection, i.e. is not older than `min_version`.
    pub fn passes_rollback_check(&self, version: KernelVersion, min_version: KernelVersion) -> bool {
        version >= min_version
    }
