...
use kernel_selector::{AbControl, KernelSelector, KernelVersion, SlotStatus};
...
impl BootManager {
    ...
    fn find_initrd_partition(&self, cmd: BootCommand, slot: SlotSuffix) -> (u64, u64) {
    ...
    pub fn mark_current_boot_successful(&mut self, slot: SlotSuffix) {
        ...
        // TODO: flush AbControl back to MISC partition via storage driver.
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn oneos_boot_manager_run(
    misc_buf:              *const u8,
    misc_len:              usize,
    persistent_flags_bits: u32,
    min_rollback_index:    u64,
...
#[no_mangle]
pub unsafe extern "C" fn oneos_mark_boot_successful(
    misc_buf:  *mut u8,
    misc_len:  usize,
    slot_index: u8,
) -> i32 {
    ...
    // In a real implementation we would re-parse, update, recompute CRC,
    // and write back.  Here we signal success without mutating the buffer.
    let _slot = if slot_index == 1 { SlotSuffix::B } else { SlotSuffix::A };
    0
}
