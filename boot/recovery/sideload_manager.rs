...
// `Default` for arrays up to length 32 in this edition (std doesn't have a
// blanket `impl<T: Default, const N: usize> Default for [T; N]`). A manual
// impl with array literals sidesteps that entirely.
...
impl SideloadManager {
    ...
    /// Current USB ADB sideload progress, if a transfer is in flight.
    /// Returns `None` when not sideloading over ADB or the size is unknown.
    pub fn adb_progress(&self) -> Option<u8> {
        if self.state != SideloadState::Receiving {
            return None;
        }
        self.adb_session.progress_pct()
    }
}
...
mod tests {
    ...
    #[test]
    fn adb_progress_none_when_idle() {
        let mgr = SideloadManager::new();
        assert_eq!(mgr.adb_progress(), None);
    }
