...
use crate::key_manager::{PublicKey, SigningAlgorithm, MAX_KEY_BYTES};
...
pub struct SignatureVerifier<'k> {
    ...
    rollback_index:  u64,
    min_rollback:    u64,
    /// Whether to enforce (true) or only log (false) failures.
    enforce:         bool,
}
...
impl<'k> SignatureVerifier<'k> {
    pub fn new(
        ...
        SignatureVerifier { signing_key, rollback_index, min_rollback, enforce }
    }
...
#[no_mangle]
pub unsafe extern "C" fn oneos_verify_kernel(image: *const u8, size: u64) -> i32 {
    ...
    // initialised earlier by oneos_secure_boot_init().
...
/// `image` must point to `size` readable bytes.
#[no_mangle]
pub unsafe extern "C" fn oneos_verify_initramfs(image: *const u8, size: u64) -> i32 {
    if image.is_null() || size == 0 {
        return -1;
    }
    let buf = core::slice::from_raw_parts(image, size as usize);
...
fn stub_verify(image: &[u8], kind: ImageKind) -> VerificationResult {
