...
// Re-export sub-modules so the C FFI shim and downstream crates can reach
// them via `monoos_secure_boot::<module>::...`.
pub mod key_manager;
...
impl BootValidator {
    ...
    pub fn validate_all<'a>(
        ...
        let verifier = SignatureVerifier::new(signing_key, rollback, min_rollback);
...
#[no_mangle]
pub unsafe extern "C" fn monoos_secure_boot_init(
...
/// `monoos_secure_boot_init` must have been called first.
#[no_mangle]
pub unsafe extern "C" fn monoos_validate_boot_images(
...
fn hash_of(data: &[u8]) -> [u8; SHA256_LEN] {
    ...
    for (i, &b) in data.iter().enumerate() {
        h[i % SHA256_LEN] = h[i % SHA256_LEN]
            .wrapping_add(b)
            .wrapping_add(0x6Bu8.wrapping_mul((i & 0xFF) as u8));
    }
    h
}
...
// ─────────────────────────────────────────────────────────────────────────────
//
// Suppressed when `external_panic_handler` is set, since build_bootloader.sh
// links this crate's staticlib together with boot_manager (which provides
// the handler) into one bootloader.elf, and only one #[panic_handler] may
// exist per link unit. See boot/boot_manager/boot_manager.rs for details.
#[cfg(all(not(feature = "std"), not(feature = "external_panic_handler")))]
#[panic_handler]
