/*
 * boot_main.c – OneOS bootloader entry point
 *
...
/* ------------------------------------------------------------------ */
extern int oneos_verify_kernel(const void *image, uint64_t size);
extern int oneos_verify_initramfs(const void *image, uint64_t size);
...
{
    serial_puts("[boot] OneOS boot stage starting\n");
...
boot_entry(uint64_t mb2_magic, uint64_t mb2_info_phys)
{
    ...
    int kv = oneos_verify_kernel(g_boot_ctx.kernel.load_addr,
                                  g_boot_ctx.kernel.size);
    if (kv != 0) boot_panic("Kernel signature verification FAILED");

    int iv = oneos_verify_initramfs(g_boot_ctx.initramfs.load_addr,
