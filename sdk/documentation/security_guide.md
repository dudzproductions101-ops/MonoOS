...
Use the **MonoOS KeyStore** instead:

```rust
use monoos_sdk::keystore::{KeyStore, KeySpec};
...
All outbound connections must use TLS 1.2 or higher.  The MonoOS network  
...
```rust
use monoos_sdk::network::HttpClient;
...
All DNS queries issued via `monoos_net_resolve()` automatically go through  
...
Run `monoos-sdk audit` before each release.  It checks your Cargo.lock  
...
```bash
monoos-sdk build --release --sign ~/.keys/release.p12
```
