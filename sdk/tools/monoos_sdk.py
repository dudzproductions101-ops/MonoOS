#!/usr/bin/env python3
"""MonoOS SDK command-line tool.

Usage:
  monoos-sdk new <package_name> [--lang rust|cpp]   Create a new app project
  monoos-sdk build [--release] [--target <abi>]     Build the current project
  monoos-sdk install [--device <serial>]            Install OPK to a device
  monoos-sdk run [--device <serial>]                Build, install, and launch
  monoos-sdk audit                                  Check for known CVEs
  monoos-sdk publish <path.opk> --api-key <key>     Upload to package repo
  monoos-sdk --version                              Print SDK version
"""
...
REPO_URL    = "https://packages.monoos.io/api/v1/upload"
...
def cmd_install(args):
    ...
    if not opk.exists():
        print("Build first: monoos-sdk build", file=sys.stderr); return 1
...
def cmd_publish(args):
    ...
    key = getattr(args, "api_key", None) or os.environ.get("MONOOS_DEV_KEY")
    if not key:
        print("Provide --api-key or set MONOOS_DEV_KEY", file=sys.stderr); return 1
...
def main():
    p = argparse.ArgumentParser(prog="monoos-sdk")
    p.add_argument("--version", action="version", version=f"monoos-sdk {SDK_VERSION}")
